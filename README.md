...
Each instruction is variable in byte length (depending on the operand mode), with the first byte containing the opcode and the operand mode, and subsequent bytes containing either the operand (in the case of _literal_ mode) or the address of the operand value to use in memory (in the case of _memory_ mode). When using a register as an operand (in _register_ mode), then the instruction will only be one byte long, as the operand mode specifies which register (A or B) to use as the operand.
